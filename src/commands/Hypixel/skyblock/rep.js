      ...
      slashOptions: [
        {
          ...
          options: [
            ...
            },
            {
              name: "user",
              description:
                "Please type the user you want to change the rep of.",
              type: "USER",
              required: true,
            },
            {
            ...
            },
            {
              name: "image",
              description:
                "An imgur link to a screenshot of a successful/unsuccessful trade or service.",
              type: "STRING",
              required: true,
            },
          ],
        },
        {
          name: "accept",
          description:
            "Declines a specific rep increase/decrease request. [ADMINS ONLY]",
          type: "SUB_COMMAND",
          options: [
            {
              name: "id",
  ...
  async exec(message, args) {
    try {
      ...
      switch (args.subcommand) {
        case "change":
          const reps = await Rep.findOne({ discordID: args.user.id });
          ...
          if (linkedAccount) {
            if (gsettings && !reps) {
              ...
              id = Rep.find({}).sort({ id: -1 }).get("id") ?? 0;
              ...
              channel.send({ embeds: [embed] });
              id = Rep.find({}).sort({ id: -1 }).get("id") ?? 0;
              const rep = await Rep.findOne({ discordID: args.user.id });
            }
          }
          break;
      }
